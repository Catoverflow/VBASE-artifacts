...
def save_recipe_data(path_image, path_text, path_text_data, path_number_data, path_json, path_result):
    with open(path_image, 'r', encoding="utf8") as f_image, \
         ...
         open(path_json, 'r', encoding="utf8") as f_json, \
        ...
        tsv_json= csv.reader(f_json, delimiter="\t")
        idx = 0
        for (id, image_embedding), (_, text_embedding), (_, ingredient, instruction), (_, ingre_count), (_, text_json) in zip(tsv_image, tsv_text, tsv_textData, tsv_numberData, tsv_json):
            ...
            text = text.replace("\'","")
            text = text.replace("%","")
            out.write(f"{id}\t{image_embedding}\t{text_embedding}\t{text}\t{str(ingre_count)}\t{text_json}\n")
            idx += 1
            if idx%10000 == 0:
...
if  __name__ == "__main__":
    ...
    parser.add_argument('--path-json', type=str, default="./simple_json.tsv",
    ...
    if os.path.exists(args.path_result):
        print("file exists")
        exit()
    save_recipe_data(args.path_image_embedding, args.path_text_embedding, args.path_text_data, args.path_number_data, args.path_json, args.path_result)
