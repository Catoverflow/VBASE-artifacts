...
def collect_gt_sel():
    ...
    fsample = open('../planning/cache/datasource_1col_sample.tsv', 'r')
    full_vecs = []
    query_vecs = []
    for line in ffull:
...
gt_cost = [[] for i in range(R_LIST_LEN)]
...
for r_sel, r_hnsw_exec, r_hnsw_esti, r_bt_exec, r_bt_esti in zip(q_r_sel, hnsw_exec, hnsw_esti, bt_exec, bt_esti):
    for q_sel, q_hnsw_exec, q_hnsw_esti, q_bt_exec, q_bt_esti in zip(r_sel, r_hnsw_exec, r_hnsw_esti, r_bt_exec, r_bt_esti):
        ...
        gt_cost[bkt_center].append(min(q_hnsw_exec, q_bt_exec))
fix_p_table = {
    "Sel(range)": [round(r/20,2) for r in range(1,11)],
    ...
    "Ground Truth": [np.mean(gt_cost[r]) for r in range(10)],
    "Count": [len(bt_cost[r]) for r in range(10)]
}
fix_p_table = "Figure 7b\n\
    Execution time when selectivity of popularity(scalar filter) is fixed to 0.9\n" +\
...
gt_cost = [[] for i in range(P_LIST_LEN)]
...
for q_sel, p in zip(q_r_sel_fix, range(len(q_r_sel_fix))):
    ...
    for i in range(P_LIST_LEN):
        ...
        gt_cost[i].append(min(hnsw_exec[i][p], bt_exec[i][p]))
...
fix_r_table = {
    "Sel(popularity)": [round((20000+20000*i)/330922, 2) for i in range(14)][:cut_index],
    ...
    "Ground Truth": [np.mean(gt_cost[i]) for i in range(cut_index)],
    "Count": [len(bt_cost[i]) for i in range(cut_index)]
}
fix_r_table = "Figure 7a\n\
    Execution time when selectivity of range query is fixed to 0.13\n" +\
