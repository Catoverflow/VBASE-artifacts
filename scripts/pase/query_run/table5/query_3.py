...
def query_generate(path_img, path_string):
    outputfile="/artifacts/scripts/pase/query_run/table5/sql/query_3_pase.sql"
    ...
    with open(path_img, 'r', encoding="utf8") as f_img, \
        ...
        for (id, image_embedding), (_, word) in zip(tsv_img, tsv_string):
            ...
            out.write("\\timing on\n")
            out.write(f"select id,image_embedding from recipe_table where (description NOT LIKE '%{word}%') order by image_embedding<*>ARRAY[{image_embedding}] limit 50;\n")
            out.write("\\timing off\n\n")
