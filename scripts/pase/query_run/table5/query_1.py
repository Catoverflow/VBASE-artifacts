...
def query_generate(path_img):
    outputfile="/artifacts/scripts/pase/query_run/table5/sql/query_1_pase.sql"
    ...
    with open(path_img, 'r', encoding="utf8") as f_data, \
        ...
        for (id, image_embedding) in tsv_data:
            ...
            out.write("\\timing on\n")
            out.write(f"select id as score from recipe_table order by image_embedding <*> ARRAY[{image_embedding}] limit 50;\n")
            out.write("\\timing off\n\n")
