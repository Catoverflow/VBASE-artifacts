...
void HNSWIndexBuilder::ConstructInternalBuilder(DistanceMethod distance_method)
{
    ...
    ranger = std::make_unique<RangeRSelEsti>();
}
...
void HNSWIndexBuilder::SaveIndex(const std::string &p_path, const std::string &ranger_path)
{
    vector_index->saveIndex(p_path);
    ranger->save(ranger_path);
}
