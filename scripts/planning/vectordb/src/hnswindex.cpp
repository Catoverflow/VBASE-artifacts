                              ...
                              IndexInfo *indexInfo)
{
    ...
    try
    {
        ...
        std::string path_for_ranger = std::string(DataDir) + std::string("/") +
                                      std::string(DatabasePath) +
                                      std::string("/ranger.bin");
        builder.SaveIndex(path, path_for_ranger);
...
IndexScanDesc hnsw_begin_scan(Relation index, int nkeys, int norderbys)
{
    std::string path = std::string(DataDir) + std::string("/") +
                       std::string(DatabasePath) + std::string("/") +
                       std::string(RelationGetRelationName(index));
    //TODO: how to get distance method and dimension
...
{
    GenericCosts costs;
    costs.numIndexTuples = -1; // pass to genericcostestimate, get real tuples there
...
#endif
    // hnsw start up cost
    // todo: call inner_product_cost() or binded support function here
    int estimated_array_length = DEFAULT_ARRAY_LENGTH, estimated_startup_steps = 100;
    // in earch search step, how many times distance function get called needed to find next point
    double dist_called_ratio = 3.2;  // we get this by testing
    double hnswinitcost = 0;
    if (path->indexclauses)
    {
        IndexClause *clause = (IndexClause *) linitial(path->indexclauses);
        Expr *expr = clause->rinfo->clause;
        if (IsA(expr, OpExpr) && list_length(((OpExpr *) expr)->args) == 2)
        {
            Node *left = (Node *) linitial(((OpExpr *) expr)->args);
            Node *right = (Node *) lsecond(((OpExpr *) expr)->args);
            if (IsA(left, Const))
            {
                // -1: current rangeR operator take first element in array as r
                estimated_array_length = estimate_array_length(left) - 1;
            }
            else if (IsA(right, Const))
            {
                estimated_array_length = estimate_array_length(right) - 1;
            }
            hnswinitcost = estimated_array_length * estimated_startup_steps *
                           dist_called_ratio * DEFAULT_CPU_OPERATOR_COST * DISTANCE_COST_COEFFI;
        }
    }
    // hnsw iterative cost
    double hnsw_iterative_cost = costs.numIndexTuples * dist_called_ratio *
                                 estimated_array_length *
                                 DEFAULT_CPU_OPERATOR_COST * DISTANCE_COST_COEFFI;
    *indexStartupCost = costs.indexStartupCost + hnswinitcost;
    *indexTotalCost = costs.indexTotalCost + hnswinitcost + hnsw_iterative_cost;
