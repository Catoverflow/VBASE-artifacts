...
CREATE FUNCTION inference2(text, text) RETURNS float8[]
	AS '/u01/app/postgres/product/13.4/lib/vectordb.so' LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;
...
CREATE FUNCTION inner_product_cost(internal) RETURNS internal
	AS '/u01/app/postgres/product/13.4/lib/vectordb.so'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

ALTER FUNCTION range_inner_product_distance(float8[], float8[]) SUPPORT inner_product_cost;
ALTER FUNCTION inner_product_distance(float8[], float8[]) SUPPORT inner_product_cost;

-- l2 product cost is not tested yet, use inner product cost estimation for the moment
ALTER FUNCTION range_l2_distance(float8[], float8[]) SUPPORT inner_product_cost;
ALTER FUNCTION l2_distance(float8[], float8[]) SUPPORT inner_product_cost;

-- selectivity estimation

CREATE FUNCTION ranger_inner_product_sel_restriction(internal, oid, internal, integer) RETURNS float8
	AS '/u01/app/postgres/product/13.4/lib/vectordb.so'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE FUNCTION ranger_inner_product_sel_join(internal, oid, internal, smallint, internal) RETURNS float8
	AS '/u01/app/postgres/product/13.4/lib/vectordb.so'
	LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

ALTER OPERATOR <<*>> (float8[], float8[]) SET (RESTRICT = ranger_inner_product_sel_restriction);

ALTER OPERATOR <<*>> (float8[], float8[]) SET (JOIN = ranger_inner_product_sel_join);

-- index handlers

CREATE FUNCTION sptag_handler(internal) RETURNS index_am_handler
	AS '/u01/app/postgres/product/13.4/lib/vectordb.so' LANGUAGE C;
