...
////////////////////////////////////////////////////////////////////////
#define PRINT(...) do{ elog(WARNING, __VA_ARGS__); } while (false)
////////////////////////////////////////////////////////////////////////
...
TupleTableSlot* execFagins(PlanState* node){
	...
	int column_num = estate->children->size();
	auto local_pq = new std::priority_queue<local_pq_item, std::vector<local_pq_item>, local_pq_item_compare_gt>();
	int *next_round = new int[column_num];
	memset(next_round, 0, sizeof(int)*column_num);
	float *score_list = new float[column_num];
	memset(score_list, 0, sizeof(float)*column_num);
	float *weight_list = new float[column_num];
	memset(weight_list, 0, sizeof(float)*column_num);
	int *count_list = new int[column_num];
	memset(count_list, 0, sizeof(int)*column_num);
	float *local_score = new float[k];
	memset(local_score, 0, sizeof(float)*k);
	int *local_id = new int[k];
	memset(local_id, 0, sizeof(int)*k);
    bool first = true;
	...
	while (!estate->finish){
		...
		if (first) {
			// at first, round-robin traverse all indices 20 times
			for (int orderby_id = 0; orderby_id < column_num; orderby_id++){
				next_round[orderby_id] = 20;
			}
			first = false;
		} else {
			// then, check top-k results coming from which index
			float total_local_score = 0.0;
			int _k = local_pq->size() > k ? k : local_pq->size(); 
			for (int i = 0; i < _k; i++){	
				local_id[i] = local_pq->top().second;
				local_score[i] = 1.0 / (1e-10 + local_pq->top().first);
				total_local_score += local_score[i];
				local_pq->pop();
			}
			for (int i = 0; i < _k; i++){	
				next_round[local_id[i]] += int(0.5 + _k*local_score[i]/total_local_score);	
			}
			// give other index chances
			float _sum = 0.0;
			for (int i = 0; i < column_num; i++){
				weight_list[i] = count_list[i] / score_list[i];
				_sum += weight_list[i];
			}
			for (int i = 0; i < column_num; i++){
				next_round[i] += int(0.5 + k*weight_list[i]/_sum);
			}
		...
		for (int i = 0; i < column_num; i++){
			for (unsigned int orderby_id = i; next_round[i] > 0; next_round[i]--){
                ...
                score_list[orderby_id] += rank_score;
