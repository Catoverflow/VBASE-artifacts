	...
	namespace SSDServing {
		namespace SSDIndex {
		    ...
		    double startTime = threadws.getElapsedMs();
			    ...
			    LOG(Helper::LogLevel::LL_Info, "Time: %f, %f\n", endTime-startTime, exEndTime - endTime);
                          //  for (int i = 1; i < p_results[index].GetResultNum(); i++) {
                           //     LOG(Helper::LogLevel::LL_Info, "result: %d\n", p_results[index].GetResult(i)->VID);
                           // }
                            //exit(0);
		    ...
		    double startTime = threadws.getElapsedMs();
                    while (true)
                    {
                        ...
                        if (index < numQueries)
                        {
                            ...
                            //LOG(Helper::LogLevel::LL_Info, "query: %d\n", index);
                            std::shared_ptr<SPANN::SPANNResultIterator<ValueType>> iterator = p_index->GetSPANNIterator(p_results[index].GetTarget(), false, 32);
                            bool hasNext = true;
                            //int i = 0;
                            while (hasNext) {
                                BasicResult result;
                                hasNext = iterator->Next(result);
				//i++;
                              //  LOG(Helper::LogLevel::LL_Info, "%d\n", i);
                               // LOG(Helper::LogLevel::LL_Info, "Result: %d\n", result.VID);
                                //if (i >= 958) break;
                            }
