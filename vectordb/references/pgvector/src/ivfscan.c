 ...
 */
static int CompareLists(const void *a, const void *b) {
  double diff =
      (((IvfflatScanList *)a)->distance - ((IvfflatScanList *)b)->distance);
  ...
  while (BlockNumberIsValid(nextblkno)) {
    ...
    for (offno = FirstOffsetNumber; offno <= maxoffno;
      ...
      distance = DatumGetFloat8(FunctionCall2Coll(
          so->procinfo, so->collation, PointerGetDatum(&list->center), value));
...
static void GetScanItems(IndexScanDesc scan, Datum value) {
  ...
  for (i = 0; i < so->probes; i++) {
    ...
    while (BlockNumberIsValid(searchPage)) {
      buf = ReadBufferExtended(scan->indexRelation, MAIN_FORKNUM, searchPage,
                               RBM_NORMAL, bas);
      ...
      for (offno = FirstOffsetNumber; offno <= maxoffno;
        ...
        slot->tts_values[1] =
            Int32GetDatum((int)ItemPointerGetBlockNumberNoCheck(&itup->t_tid));
        slot->tts_isnull[1] = false;
        slot->tts_values[2] =
            Int32GetDatum((int)ItemPointerGetOffsetNumberNoCheck(&itup->t_tid));
...
IndexScanDesc ivfflatbeginscan(Relation index, int nkeys, int norderbys) {
  ...
  so = (IvfflatScanOpaque)palloc(offsetof(IvfflatScanOpaqueData, lists) +
                                 lists * sizeof(IvfflatScanList));
  ...
  /* Prep sort */
#if PG_VERSION_NUM >= 110000
  so->sortstate = tuplesort_begin_heap(so->tupdesc, 1, attNums, sortOperators,
                                       sortCollations, nullsFirstFlags,
                                       work_mem, NULL, false);
#else
  so->sortstate =
      tuplesort_begin_heap(so->tupdesc, 1, attNums, sortOperators,
                           sortCollations, nullsFirstFlags, work_mem, false);
#endif
 ...
 */
void ivfflatrescan(IndexScanDesc scan, ScanKey keys, int nkeys,
...
bool ivfflatgettuple(IndexScanDesc scan, ScanDirection dir) {
...
#endif
  {
    ...
    BlockNumber indexblkno =
        DatumGetInt32(slot_getattr(so->slot, 4, &so->isnull));
