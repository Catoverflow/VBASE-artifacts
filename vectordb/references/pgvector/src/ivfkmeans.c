 ...
 */
static void InitCenters(Relation index, VectorArray samples,
                        VectorArray centers, double *lowerBound) {
  FmgrInfo *procinfo;
  Oid collation;
  int i;
  int j;
  ...
  VectorArraySet(centers, 0,
                 VectorArrayGet(samples, random() % samples->length));
  centers->length++;
  ...
  for (i = 0; i < numCenters; i++) {
    ...
    for (j = 0; j < numSamples; j++) {
      ...
      distance = DatumGetFloat8(
          FunctionCall2Coll(procinfo, collation, PointerGetDatum(vec),
 ...
 */
static inline void ApplyNorm(FmgrInfo *normprocinfo, Oid collation,
                             Vector *vec) {
  int i;
  double norm = DatumGetFloat8(
      FunctionCall1Coll(normprocinfo, collation, PointerGetDatum(vec)));
 ...
 */
static void QuickCenters(Relation index, VectorArray samples,
                         VectorArray centers) {
  int i;
  int j;
  Vector *vec;
  ...
  qsort(samples->items, samples->length, VECTOR_SIZE(samples->dim),
        CompareVectors);
...
/*
 * Use Elkan for performance. This requires distance function to satisfy
 * triangle inequality.
 *
 ...
 */
static void ElkanKmeans(Relation index, VectorArray samples,
                        VectorArray centers) {
  FmgrInfo *procinfo;
  FmgrInfo *normprocinfo;
  Oid collation;
  ...
  for (iteration = 0; iteration < 500; iteration++) {
    ...
    for (j = 0; j < numCenters; j++) {
      ...
      for (k = j + 1; k < numCenters; k++) {
        distance = 0.5 * DatumGetFloat8(FunctionCall2Coll(
                             procinfo, collation, PointerGetDatum(vec),
      ...
      for (k = 0; k < numCenters; k++) {
        ...
        if (rj) {
          dxcx = DatumGetFloat8(FunctionCall2Coll(
              procinfo, collation, PointerGetDatum(vec),
              PointerGetDatum(VectorArrayGet(centers, closestCenters[j]))));
    ...
    /* Step 5 */
    for (j = 0; j < numCenters; j++)
      newcdist[j] = DatumGetFloat8(FunctionCall2Coll(
          procinfo, collation, PointerGetDatum(VectorArrayGet(centers, j)),
    ...
    /* Step 7 */
    for (j = 0; j < numCenters; j++)
      memcpy(VectorArrayGet(centers, j), VectorArrayGet(newCenters, j),
 ...
 */
static void CheckCenters(Relation index, VectorArray centers) {
  FmgrInfo *normprocinfo;
  Oid collation;
  int i;
  double norm;
  ...
  qsort(centers->items, centers->length, VECTOR_SIZE(centers->dim),
        CompareVectors);
  for (i = 1; i < centers->length; i++) {
    if (CompareVectors(VectorArrayGet(centers, i),
                       VectorArrayGet(centers, i - 1)) == 0)
  ...
  if (normprocinfo != NULL) {
    ...
    for (i = 0; i < centers->length; i++) {
      norm = DatumGetFloat8(
          FunctionCall1Coll(normprocinfo, collation,
                            PointerGetDatum(VectorArrayGet(centers, i))));
      if (norm == 0)
