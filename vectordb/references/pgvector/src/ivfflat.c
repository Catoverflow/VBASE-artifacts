...
void _PG_init(void) {
  ...
  DefineCustomIntVariable("ivfflat.probes", "Sets the number of probes",
                          "Valid range is 1..lists.", &ivfflat_probes, 1, 1,
 ...
 */
static void ivfflatcostestimate(PlannerInfo *root, IndexPath *path,
...
static bytea *ivfflatoptions(Datum reloptions, bool validate) {
  ...
  options =
      parseRelOptions(reloptions, validate, ivfflat_relopt_kind, &numoptions);
 ...
 */
static bool ivfflatvalidate(Oid opclassoid) { return true; }
...
Datum ivfflathandler(PG_FUNCTION_ARGS) {
  ...
  amroutine->amparallelvacuumoptions =
      VACUUM_OPTION_NO_PARALLEL; /* TODO support parallel */
#endif
