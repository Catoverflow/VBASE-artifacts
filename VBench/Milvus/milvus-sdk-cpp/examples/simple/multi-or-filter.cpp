...
main(int argc, char* argv[]) {
    ...
    std::string output_result_path = outputResult + std::to_string(search_limit) +"qrels-filter.txt";
    std::string output_lantency_path = outputResult + std::to_string(search_limit) + "latency-filter.txt";
    ...
    for (int i = 0; i < SIZE_QUERY; i++) {
        ...
        while (limit <= search_limit && limit <= SIZE_COLLECTION) {
            ...
            ins_arguments.SetExpression("ingredient_count <= " + std::to_string(ingre_num[i]) + " || instruction_step <= " + std::to_string(instruct_step[i]));
